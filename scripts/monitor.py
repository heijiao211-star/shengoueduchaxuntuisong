...
def fetch_fund(fund: dict) -> dict:
    ...
    try:
        ...
        if "暂停申购" in raw_status or "封闭" in raw_status:
            ...
            suggestion = min(int(fund["planned_amount"]), cap)
        elif "开放" in raw_status or "申购" in raw_status:
            subscription_status = "开放申购"
            suggestion = int(fund["planned_amount"])
        else:
...
def build_markdown(rows: list[dict], checked_at: str) -> str:
    available = [row for row in rows if row["suggested_amount"] > 0]
    total = sum(row["suggested_amount"] for row in rows)
    lines = [
        "# 纳指100场外申购额度日报",
        "",
        f"> 更新：{checked_at}（北京时间）  ",
        f"> 可申购：**{len(available)}/{len(rows)}** 只 ｜ 按计划可申购合计：**¥{total:,}**",
        "",
        "| 代码 | 基金 | 基金公司 | 定投标记 | 计划申购 | 今日状态 | 单日上限 | 今日建议申购 | 定投状态 |",
        "|---|---|---|---|---:|---|---:|---:|---|",
    ]
    for row in rows:
        lines.append(
            f"| {row['code']} | {row['name']} | {row['manager']} | {invested(row['invested'])} | "
            f"{money(row['planned_amount'])} | {row['subscription_status']} | {money(row['daily_limit'])} | "
            f"{money(row['suggested_amount'])} | {row['sip_status']} |"
        )
    lines += [
        "",
        "说明：数据来自公开基金档案的交易状态；“今日建议申购”=计划申购金额与公开单日上限的较小值。支付宝最终下单结果以支付宝页面为准。",
    ]
    return "\n".join(lines) + "\n"
...
def build_html(rows: list[dict], checked_at: str) -> str:
    ...
    for row in rows:
        table_rows.append(
            ...
            f"<td>{money(row['planned_amount'])}</td>"
            ...
            f"<td class='amount'>{money(row['suggested_amount'])}</td>"
...
</style></head><body><div class='wrap'><section class='hero'><h1>纳指100 · 场外申购额度日报</h1><p>{html.escape(checked_at)}（北京时间）｜公开基金状态监控</p></section><section class='stats'><div class='stat'><span>可申购基金</span><b>{len(available)} / {len(rows)}</b></div><div class='stat'><span>按计划建议申购</span><b>¥{total:,}</b></div></section><section class='table'><table><thead><tr><th>代码 / 公司</th><th>基金 / 标记</th><th>计划申购</th><th>今日状态</th><th>单日上限</th><th>建议申购</th><th>定投状态</th></tr></thead><tbody>{''.join(table_rows)}</tbody></table></section><p class='foot'>“建议申购”取计划申购金额与公开单日上限的较小值；支付宝最终下单结果以支付宝页面为准。</p></div></body></html>"""
...
def write_history(rows: list[dict], checked_at: str) -> None:
    ...
    with path.open("a", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["checked_at", "code", "name", "status", "daily_limit", "suggested_amount", "sip_status", "source_url", "error"])
        if new_file:
            writer.writeheader()
        for row in rows:
            writer.writerow({"checked_at": checked_at, "code": row["code"], "name": row["name"], "status": row["subscription_status"], "daily_limit": row["daily_limit"], "suggested_amount": row["suggested_amount"], "sip_status": row["sip_status"], "source_url": row["source_url"], "error": row["error"]})
